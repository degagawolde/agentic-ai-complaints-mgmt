...
# 1) Recommended folder structure (monorepo)
...
**Quick notes about folders**

* `api` = public API that receives complaints, returns status, and triggers orchestration (calls the AI service).
* `ai-orchestration` = the brain: LangGraph flows, LangChain tools (SQL tool, vector retriever, OpenAI wrapper), prompt library.
* `worker` = background workers for sending emails, SMS, retry logic, and long-running indexing.
* `web` = React dashboard for departments and admins.
* `scripts/seed_kb.py` = processes policy docs, splits, creates embeddings, and writes them to the vector DB.
* `docs/` = reference prompts, runbooks, and diagrams for auditors.
